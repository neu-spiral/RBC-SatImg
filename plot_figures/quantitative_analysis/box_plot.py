...
def set_layout_bp(axs, axs_i, axs_j):
...
if __name__ == "__main__":
    ...
    for i, test_site_i in enumerate(test_sites):
        ...
        bp = axs[axs_i, axs_j].boxplot(results_qa_sorted, positions=positions, labels=models_sorted, notch=False, patch_artist=True, vert=True)
        set_style_bp(bp=bp)
        # Move xticks to right
        set_layout_bp(axs, axs_i, axs_j)
    #
    #
    # Set bp legend
    set_legend_bp()
    ...
    # --------------------------------------------------------------------
    if savefig:
        path_store = os.path.join(Config.path_evaluation_results, "classification", "figures", "boxplot.svg")
