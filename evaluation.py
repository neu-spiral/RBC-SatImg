...
def evaluate_models(image_all_bands: np.ndarray, rbc_objects: Dict[str, RBC], time_index: int, image_index: int, date_string: str):
    ...
    pickle_file_path = os.path.join(Config.path_evaluation_results, f'{Config.scenario}_{Config.scene_id}',
    ...
    if Debug.evaluation_results_pickle:
        ...
        # GMM Model
        y_pred["GMM"], predicted_image["GMM"], prediction_float["GMM"] = rbc_objects["GMM"].update_labels(image_all_bands=image_all_bands,
                                                                                 time_index=time_index)
        predicted_image["GMM"] = predicted_image["GMM"].reshape(dim_h, dim_v)
        y_pred["GMM"] = y_pred["GMM"].reshape(dim_h, dim_v)

        # Scaled Index Model
        y_pred["Scaled Index"], predicted_image["Scaled Index"], prediction_float["Scaled Index"] = rbc_objects["Scaled Index"].update_labels(
        ...
        y_pred["Logistic Regression"], predicted_image["Logistic Regression"], prediction_float["Logistic Regression"] = rbc_objects[
        ...
        if Config.scenario == "oroville_dam":
            ...
            y_pred["DeepWaterMap"], predicted_image["DeepWaterMap"], prediction_float["DeepWaterMap"] = rbc_objects["DeepWaterMap"].update_labels(
                image_all_bands=image_all_bands, time_index=time_index)
    ...
    # If user wants to load evaluation plot_results (to skip evaluation, as it might take long, if user just wants to plot previously saved plot_results)
...
def evaluation_main(gmm_densities: List[GaussianMixture], trained_lr_model: LogisticRegression,
    ...
    for image_idx in range(Config.offset_eval_images, Config.offset_eval_images + num_evaluation_images):
        ...
        if not Debug.check_dates:
            ...
            y_pred, predicted_image = evaluate_models(image_all_bands=image_all_bands, rbc_objects=rbc_objects,
                                                      time_index=time_index, image_index=image_idx, date_string=date_string)

            # Plot plot_results for this image
            print(f"plotting plot_results for image with index {image_idx}")
