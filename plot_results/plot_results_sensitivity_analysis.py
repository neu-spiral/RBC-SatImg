...
path_files = '/Users/helena/Documents/Research/Recursive_Bayesian_Image_Classification/0_rse_isprs_submissions/RBC-SatImg/evaluation_results/sensitivity_analysis'
results = dict()
...
for idx_model, model in enumerate(models_to_plot):
    ...
    for epsilon_i in epsilon_to_plot:
        if epsilon_i == 1:
            list_axis[idx_model].plot(dates_array[1:], results_sorted[epsilon_benchmark][model[1:]], linewidth=1,
                                  linestyle=linestyles[plot_count], marker=markerstyles[plot_count],
                                  markersize=marker_sizes[plot_count], color=colors[plot_count])
        else:
            list_axis[idx_model].plot(dates_array[1:], results_sorted[epsilon_i][model], linewidth=1,
                                  linestyle=linestyles[plot_count], marker=markerstyles[plot_count],
                                  markersize=marker_sizes[plot_count], color=colors[plot_count])
    ...
    list_axis[idx_model].set_xticklabels(list_axis[idx_model].get_xticklabels(), rotation=45, ha="right", fontsize=13, family='Times New Roman')
    ...
    # Add x and y labels
    #list_axis[idx_model].set_title(model, fontsize=21, fontname='Times New Roman')
    list_axis[idx_model].set_ylabel('Water pixels', fontsize=20, fontname='Times New Roman')
...
list_axis[0].set_yticklabels([2000, 4000, 6000, 8000, 10000], fontsize=17,
                                     family='Times New Roman')
list_axis[1].set_yticklabels([3000, 5000, 7000, 9000], fontsize=17,
                                     family='Times New Roman')
list_axis[2].set_yticklabels([1000, 3000, 5000, 7000, 9000], fontsize=17,
                                     family='Times New Roman')
...
leg = axarr[0].legend(epsilon_to_plot_legend, ncol=5, loc='upper center', bbox_to_anchor=(0.5,2),
                  prop=font)
...
if Debug.save_figures:
    f1.savefig(os.path.join(Config.path_results_figures, 'sensitivity_analysis_RGMM.pdf'), format="pdf",
               bbox_inches="tight")

    f2.savefig(os.path.join(Config.path_results_figures, 'sensitivity_analysis_RLR.pdf'), format="pdf", bbox_inches="tight")
    f3.savefig(os.path.join(Config.path_results_figures, 'sensitivity_analysis_RSIC.pdf'), format="pdf", bbox_inches="tight")
...
# Save figure as pdf
if Debug.save_figures:
    plt.savefig(os.path.join(Config.path_results_figures, 'sensitivity_analysis.pdf'), format="pdf", bbox_inches="tight")"""